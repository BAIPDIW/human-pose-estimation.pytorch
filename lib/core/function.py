...
from utils.transforms import flip_back,fliplr_joints
...
def euclidean_losses(actual, target,coords_vis):
    ...
    #logger.info('-------------------------------------coords_vis-----------------------------------------------')
    #logger.info(coords_vis[0])
    #diff = diff.mul(coords_vis)
    #logger.info('-------------------------------------diff.mul(coords_vis)-------------------------------------')
    #logger.info(diff[0])
...
def train_softargmax(config, train_loader, model, criterion, optimizer, epoch,
    ...
    for i, (input, target, target_weight, meta,coords,coords_vis) in enumerate(train_loader):
        # measure data loading time
        data_time.update(time.time() - end)
        # compute output
        coords_pred,output = model(input)
        
        coords = coords.cuda(non_blocking=True)  #(batch_size,17,2)
        coords_vis =coords_vis.float()          #(batch_size,17,2)
        coords_vis = coords_vis.cuda(non_blocking=True) 
        #print('coords_pred=',coords_pred[0][0])
        #print('coords=',coords[0][0])
        euc_losses = euclidean_losses(coords_pred,coords,coords_vis)
...
def get_final_preds_softargmax(config, batch_heatmaps, center, scale,coords_pred):
